...
- **Wazuh manager restart** (`sudo /var/ossec/bin/wazuh-control restart`, needed to activate `MITRE_MAPPING.md`'s rule fixes): confirmed genuinely blocked — `sudo -n` requires an interactive password (`sudo -ln` also refused non-interactively), and no NOPASSWD sudoers entry exists. **Not attempted further; not claimed as done.** *Planned:* perform the restart with the user's own sudo password in the next session that has interactive access to this environment.
...
| Neo4j | IMPLEMENTED + TESTED | `neo4j_client.py` | full suite depends on it | **Yes** — was down, started this phase, full 643-test suite passed cleanly against it | Was not running at session start; now running, not persisted across a host reboot without the user re-running `docker start neo4j-soc`. *Planned:* configure the container with a restart policy (e.g. `--restart unless-stopped`) so it survives a host reboot without manual intervention. |
...
| Shuffle | IMPLEMENTED + ENVIRONMENT BLOCKED | `soar/shuffle_client.py` | tested with mocked HTTP | **No** — `SHUFFLE_WEBHOOK` still unconfigured in this environment | Unchanged from the prior SOAR phase; no live execution has ever occurred. *Planned:* run one real playbook execution once `SHUFFLE_WEBHOOK` is configured in this environment. |
...
- The rule `100510` ("Multiple Failed Login Attempts... SSH brute force") has no `frequency`/`timeframe` repetition threshold despite its description — flagged in `MITRE_MAPPING.md`, not fixed this phase (out of scope: changing detection thresholds is a different risk category than fixing a MITRE mapping). *Planned:* add the missing `frequency`/`timeframe` threshold to rule `100510` in a session scoped to review detection thresholds.
...
- Shuffle/authenticated-MISP live execution — infrastructure/credential blocked, not something local implementation can close further. *Planned:* reconfirm both once `SHUFFLE_WEBHOOK`/`MISP_API_KEY` are configured for a verification session.
- Wazuh manager restart — privileged-access blocked. *Planned:* perform the restart with the user's own sudo password in the next interactive session.
- The STIX corpus data-quality issue — needs the user's decision (re-fetch vs. investigate further) rather than an autonomous fix. *Planned:* re-fetch and diff the official bundle once the user decides to pursue this.
...
- No accuracy dashboard, by design (would show numbers with no real ground truth behind them). *Planned:* build one once any (C) row in `ACCURACY_EVALUATION.md` gets real, independently-labeled samples.
