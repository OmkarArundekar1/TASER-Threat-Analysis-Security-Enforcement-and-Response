...
|---|---|---|---|
| 1 | CYUKTI integrates heterogeneous cybersecurity telemetry into a campaign-centric graph | **STRONGLY DEMONSTRATED** | Live graph: 65 Campaigns, 124 AttackEvents, 858 Technique nodes, 5 Attacker, 4 Host, 43 Operation nodes, 119 MATCHES relationships, 0 orphans. This is real accumulated Wazuh-sourced telemetry (not synthetic), structured around Campaign as the central object, with 150/150 tests passing on the ingestion/graph pipeline. |
...
| 4 | Campaign reconstruction repairs real graph-integrity failures | **STRONGLY DEMONSTRATED** | A real, found-in-production defect (44 AttackEvents with no parent Campaign) was root-caused, repaired via evidence-only reconstruction (never guessing), proven idempotent by 10 passing tests, and the repair still holds today (0/124 orphans). This is a genuine before/after real-data result, not a synthetic demonstration. |
