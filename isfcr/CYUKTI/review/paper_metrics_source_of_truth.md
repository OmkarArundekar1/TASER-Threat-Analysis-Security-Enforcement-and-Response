...
> Last verified: 2026-09-26. This file is the authoritative status list for the 7 metric categories the evaluation audit identified as missing. It supersedes the "0/0" and "NOT MEASURED" placeholders in `review/paper_submission_status.md` for these specific rows — see that file for the rest of the paper's numbers, which this task did not touch. Four categories only: **MEASURED**, **MEASURED_PRELIMINARY**, **NOT MEASURED**, **UNMEASURABLE**, **BLOCKED_BY_ENVIRONMENT**. Never silently converted between them.
...
| Threat qualification accuracy | **MEASURED_PRELIMINARY** | Accuracy=0.083 | 24 | AUTO_PROPOSED. Large, real disagreement between per-scenario expected status and CYUKTI's per-campaign CTI classification — disclosed as an open question, not resolved. |
