...
describe('DashboardProvider -- Open Explore Path navigation', () => {
  function ExploreConsumer() {
    const { activeView, selectedCampaign, openExplorePath, setActiveView, loading } = useDashboard();
    return (
      <div>
        <div data-testid="active-view">{activeView}</div>
        <div data-testid="selected">{selectedCampaign ?? 'none'}</div>
        <div data-testid="loading">{String(loading)}</div>
        <button onClick={() => openExplorePath('CAMP_C516A844')}>open-explore-path</button>
        <button onClick={() => setActiveView('dashboard')}>back-to-overview</button>
      </div>
    );
  }

  it('starts on the dashboard view with no campaign selected', async () => {
    mockAllEndpointsHealthy();
    render(<DashboardProvider><ExploreConsumer /></DashboardProvider>);
    expect(screen.getByTestId('active-view').textContent).toBe('dashboard');
    expect(screen.getByTestId('selected').textContent).toBe('none');
    await waitFor(() => expect(screen.getByTestId('loading').textContent).toBe('false'));
  });

  it('openExplorePath selects the campaign and switches to the incident view in one step, preserving the canonical campaign id', async () => {
    mockAllEndpointsHealthy();
    render(<DashboardProvider><ExploreConsumer /></DashboardProvider>);
    await waitFor(() => expect(screen.getByTestId('loading').textContent).toBe('false'));

    await act(async () => {
      screen.getByRole('button', { name: 'open-explore-path' }).click();
    });
    await waitFor(() => expect(screen.getByTestId('loading').textContent).toBe('false'));

    expect(screen.getByTestId('active-view').textContent).toBe('incident');
    expect(screen.getByTestId('selected').textContent).toBe('CAMP_C516A844');
  });

  it('setActiveView(\'dashboard\') returns to Overview without losing the selected campaign', async () => {
    mockAllEndpointsHealthy();
    render(<DashboardProvider><ExploreConsumer /></DashboardProvider>);
    await waitFor(() => expect(screen.getByTestId('loading').textContent).toBe('false'));
    ...
    await waitFor(() => expect(screen.getByTestId('loading').textContent).toBe('false'));
    await act(async () => {
      screen.getByRole('button', { name: 'back-to-overview' }).click();
    });
